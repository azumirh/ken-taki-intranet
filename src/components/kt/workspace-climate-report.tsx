...
function jpegToPdf(jpegDataUrl: string, width: number, height: number) {
  ...
  imageObject.set(imageHead, 0);
  imageObject.set(imgBytes, imageHead.length);
  imageObject.set(imageTail, imageHead.length + imgBytes.length);
...
export function WorkspaceClimateReport({ mode }: { mode: Mode }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast.error((error as Error).message || "Não foi possível carregar o clima.");
    } finally {
      setLoading(false);
    }
  ...
  // All hooks above this point must run on every render. In the initial render the
  // authenticated profile can still be loading; returning earlier caused React #310.
  ...
  function exportCsv() {
    ...
    const lines = [
      ...
      ...rows.map((row) =>
        [
          row.nome,
          filialNome(row.filial),
          row.humor,
          categoryFor(row.humor),
  ...
  function exportPdf() {
    ...
    ctx.fillText("Ken Taki · Relatório de clima", 72, 92);
    ctx.fillStyle = "#d9ccd4";
    ...
    ctx.fillStyle = "#2b2729";
    ctx.font = "700 28px Segoe UI, Arial";
    ctx.fillText("Resumo do período", 72, 315);
    const cards = [
      ["Registros", summary.total, "#4b3142"],
      ["Pessoas", summary.uniquePeople, "#66505d"],
      ["Positivos", summary.positive, "#39715a"],
      ["Negativos", summary.negative, "#a65a42"],
    ] as const;
    cards.forEach(([label, value, color], index) => {
      const x = 72 + index * 274;
      ctx.fillStyle = "#ffffff";
      ctx.fillRect(x, 350, 242, 132);
      ctx.strokeStyle = "#ddd6d0";
      ctx.strokeRect(x, 350, 242, 132);
      ctx.fillStyle = color;
      ctx.fillRect(x, 350, 242, 7);
      ctx.fillStyle = "#2b2729";
      ctx.font = "700 38px Segoe UI, Arial";
      ctx.fillText(String(value), x + 22, 408);
      ctx.fillStyle = "#6f6966";
      ctx.font = "20px Segoe UI, Arial";
      ctx.fillText(label, x + 22, 447);
    });
    const pct = (value: number) => (summary.total ? Math.round((value / summary.total) * 100) : 0);
    ctx.fillStyle = "#2b2729";
    ctx.font = "700 28px Segoe UI, Arial";
    ctx.fillText("Distribuição", 72, 550);
    const distributions = [
      ["Positivo", summary.positive, "#39715a"],
      ["Neutro", summary.neutral, "#9d8d82"],
      ["Negativo", summary.negative, "#a65a42"],
    ] as const;
    distributions.forEach(([label, value, color], index) => {
      const y = 590 + index * 66;
      ctx.fillStyle = "#e8e2dd";
      ctx.fillRect(210, y, 800, 25);
      ctx.fillStyle = color;
      ctx.fillRect(210, y, (800 * pct(value)) / 100, 25);
      ctx.fillStyle = "#403a3d";
      ctx.font = "20px Segoe UI, Arial";
      ctx.fillText(label, 72, y + 20);
      ctx.fillText(`${value} · ${pct(value)}%`, 1030, y + 20);
    });
    ctx.fillStyle = "#2b2729";
    ctx.font = "700 28px Segoe UI, Arial";
    ctx.fillText("Leitura executiva", 72, 825);
    ctx.fillStyle = "#5e5754";
    ctx.font = "22px Segoe UI, Arial";
    const interpretation =
      summary.total === 0
        ? "Não houve registros no período selecionado."
        : `Foram registrados ${summary.total} check-ins por ${summary.uniquePeople} pessoas. ${pct(summary.positive)}% foram positivos, ${pct(summary.neutral)}% neutros e ${pct(summary.negative)}% negativos. O relatório deve ser lido como sinal de clima e não como diagnóstico individual.`;
    let nextY = wrapText(ctx, interpretation, 72, 870, 1090, 34, 5) + 30;
    ctx.fillStyle = "#2b2729";
    ctx.font = "700 28px Segoe UI, Arial";
    ctx.fillText("Evolução diária", 72, nextY);
    nextY += 48;
    ctx.font = "18px Segoe UI, Arial";
    daily.slice(-18).forEach((day, index) => {
      const y = nextY + index * 38;
      ctx.fillStyle = index % 2 ? "#f2efeb" : "#ffffff";
      ctx.fillRect(72, y - 24, 1090, 34);
      ctx.fillStyle = "#4a4441";
      ctx.fillText(day.date, 92, y);
      ctx.fillText(`Positivos ${day.Positivo}`, 300, y);
      ctx.fillText(`Neutros ${day.Neutro}`, 560, y);
      ctx.fillText(`Negativos ${day.Negativo}`, 800, y);
    });
    ctx.fillStyle = "#8a817b";
    ctx.font = "16px Segoe UI, Arial";
    ctx.fillText(`Gerado em ${new Date().toLocaleString("pt-BR")} · Intranet Ken Taki`, 72, 1680);
      ...
      <div className="flex flex-col gap-4 border-b border-border px-5 py-5 sm:flex-row sm:items-start sm:justify-between lg:px-6">
        <div>
          <div className="flex items-center gap-2">
            <SlidersHorizontal className="h-4 w-4 text-kt" />
            <h2 className="text-lg font-bold">Clima por período</h2>
          </div>
          <p className="mt-1 text-sm text-muted-foreground">
            Filtre a leitura e gere um relatório independente do que estiver visível na tela.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" size="sm" disabled={loading || rows.length === 0} onClick={exportCsv}>
            <Download className="h-3.5 w-3.5" /> CSV
          </Button>
          <Button size="sm" disabled={loading} onClick={exportPdf}>
            <FileText className="h-3.5 w-3.5" /> Baixar relatório PDF
          </Button>
        </div>
      </div>
      ...
      <div className="grid gap-5 p-5 lg:p-6">
        <div className="flex flex-wrap items-end gap-3">
          <div className="flex flex-wrap gap-1 rounded-md bg-muted p-1">
            {(["7d", "30d", "month"] as const).map((value) => (
              <button
                ...
                className={`rounded px-3 py-1.5 text-xs font-semibold ${preset === value ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"}`}
              >
                {value === "7d" ? "7 dias" : value === "30d" ? "30 dias" : "Este mês"}
              </button>
            ))}
            <button
              onClick={() => setPreset("custom")}
              className={`rounded px-3 py-1.5 text-xs font-semibold ${preset === "custom" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"}`}
            >
              Período específico
            </button>
          </div>
          ...
          {mode === "hr" ? (
            <div>
              <Label>Unidade</Label>
              <select
                className="mt-1 h-9 rounded-md border border-border bg-card px-3 text-sm"
                value={filial}
        ...
        <div className="grid gap-2 sm:grid-cols-4">
          {[
            ...
            ["Pessoas", summary.uniquePeople],
            ["Positivos", summary.positive],
            ["Negativos", summary.negative],
          ].map(([label, value]) => (
        ...
        <div className="h-[260px] min-w-0 rounded-lg border border-border bg-card p-3">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={daily}>
              ...
              <Tooltip />
              <Bar dataKey="Positivo" stackId="a" fill="#39715a" radius={[2, 2, 0, 0]} />
              <Bar dataKey="Neutro" stackId="a" fill="#9d8d82" />
              <Bar dataKey="Negativo" stackId="a" fill="#a65a42" radius={[0, 0, 2, 2]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
