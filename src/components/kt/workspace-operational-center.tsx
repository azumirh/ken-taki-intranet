...
import { BellRing, BriefcaseBusiness, CheckCircle2, ClipboardList, HeartHandshake, History, ShieldCheck, UserMinus, UserRoundCheck, UsersRound } from "lucide-react";
...
function FieldSelect({ label, value, onChange, children }: { label: string; value: string; onChange: (value: string) => void; children: React.ReactNode }) {
  return <div className="grid gap-1.5"><Label>{label}</Label><select value={value} onChange={(event) => onChange(event.target.value)} className="h-10 rounded-md border border-input bg-background px-3 text-sm text-foreground">{children}</select></div>;
}
...
export function WorkspaceOperationalCenter({ mode }: { mode: Mode }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      let peopleQuery = supabase.from("kt_colaboradores").select("id,nome,filial,cargo,ativo").order("nome");
      ...
      let surveyQuery = supabase.from("kt_pesquisas").select("id,titulo,descricao,prazo,ativa,modo,anonima,filial_alvo,encerrada_em,ts").eq("modo", "interna").order("ts", { ascending: false }).limit(50);
      if (branch) {
        peopleQuery = peopleQuery.eq("filial", branch);
      ...
      const [peopleResult, caseResult, offResult, vacancyResult, recResult, onResult, surveyResult] = await Promise.all([peopleQuery, caseQuery, offQuery, vacancyQuery, recQuery, onQuery, surveyQuery]);
      ...
      const summaries = await Promise.all(nextSurveys.map(async (survey) => {
        const { data } = await supabase.rpc("kt_internal_survey_summary", { p_pesquisa_id: survey.id, p_filial: branch });
  ...
  async function saveCase() {
    ...
    try {
      const statusAllowed = selectedCase.origem_tabela === "kt_feedbacks" || selectedCase.origem_tabela === "kt_ajuda" || !selectedCase.origem_tabela;
      const { error } = await supabase.rpc("kt_update_case_workflow", {
        p_case_id: selectedCase.id,
        p_status: statusAllowed ? caseStatus : null,
  ...
  async function createVacancy() {
    ...
    const branch = mode === "manager" ? profile.filial : profile.filial ?? "champagnat";
    ...
    {can("casos", "view") ? <section id="casos" className="surface scroll-mt-24 overflow-hidden">
      ...
      <div className="grid gap-4 p-5 lg:grid-cols-[minmax(260px,.8fr)_minmax(0,1.4fr)] lg:p-6">
        ...
        {selectedCase ? <div className="grid gap-4 rounded-xl border border-border bg-card p-4 sm:p-5">
          ...
          <div className="grid gap-3 sm:grid-cols-2"><FieldSelect label="Status" value={caseStatus} onChange={setCaseStatus}>{STATUS_CASE.map((status) => <option key={status} value={status}>{status.replaceAll("_", " ")}</option>)}</FieldSelect><div className="grid gap-1.5"><Label>Responsável operacional</Label><div className="flex h-10 items-center rounded-md border border-input bg-muted/40 px-3 text-sm">{profile.nome}</div></div></div>
